...
- **`<X>Screen.kt`** — `@Composable fun <X>Screen(vm: <X>ViewModel = koinViewModel())` → `val state by vm.state.collectAsStateWithLifecycle()` then `<X>Content(state, <X>Callbacks(onRowClick = vm::..., ...))`. **Always `collectAsStateWithLifecycle`** (battery).
- **`<Feat>NavGraph.kt`** — `fun NavGraphBuilder.<feat>Graph(navController, onLogout, pendingSyncCount, role, user, onNavigateMain, onProfileClick)` with `composable<<Feat>> { ShelledScreen(title = "ไทย", ...) { <X>Screen() } }`.
