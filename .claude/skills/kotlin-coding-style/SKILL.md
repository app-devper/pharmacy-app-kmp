...
| ktlint / Detekt | Often **not configured**. Strict-KMP projects use a custom `auditArchitecture` Gradle task (wired into `check`) that enforces project-specific architecture rules — layering, DTO conventions, platform-folder ownership, no `expect`, no generic exceptions, no un-localized copy — instead of generic style rules. Note what it does **not** catch: comments, `!!`, file length and the file-per-class rule are review-time, not build-enforced. | See `.claude/skills/kmp-build-logic/` §3 for the audit task skeleton and `.claude/skills/kmp-review/` for the rule catalog. |
...
| `Screen.kt`, `Content.kt`, `ViewModel.kt`, `UiState.kt` may share a file when small | **File-per-class**: each must live in its own file. Review-time rule — the audit task does not grep for it. | See `.claude/skills/kmp-code-pattern/` §2. |
