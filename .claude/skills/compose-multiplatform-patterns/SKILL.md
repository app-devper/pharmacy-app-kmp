...
> Projects with a hand-curated brand palette **disable** dynamic theming (it would override the
> brand colors) and read tokens from a custom tokens data class via a `CompositionLocal` — here
> `PharmTokens` behind `LocalPharmTokens`. See `.claude/skills/kmp-design-system/` for the token + primitives
> approach.
...
| `MaterialTheme(colorScheme = dynamicLightColorScheme(...))` | Use `PharmacyTheme { ... }` which provides `LocalPharmTokens` (a `PharmTokens(colors, spacing, radii, shapes, dimens, fontScale)` data class) — production code reads `pharmTokens.colors.accent` not `MaterialTheme.colorScheme.primary`. It still wraps `MaterialTheme` so M3 internals (ripple, IME insets, sheet scrims) inherit the palette. **No raw M3 widgets in new files**: use `PharmButton` / `PharmBadge` / `PharmTextField` / `FormField` / `PharmModal` / `MetricCard` / `PharmTable` / `PharmFilterChips` / `PharmActionMenu` / `PharmTabBar` / `PharmStatusBadge` / `PharmDateRangeField` from `:core:ui/.../designsystem/`. | See `.claude/skills/kmp-design-system/`. The design follows a hand-curated palette; dynamic Material You theming would override and clash with it. |
...
| `Screen.kt` + `Content.kt` + `ViewModel.kt` + `UiState.kt` may live in the same file when small | **File-per-class**: each must live in its own file. This one is a review-time rule, not build-enforced — the audit task does not grep for it. | See `.claude/skills/kmp-code-pattern/` §2. |
| Page header is custom per screen (Topbar / Toolbar / breadcrumb) | List pages are `PharmListScaffold(toolbar, metrics?, resultLine, content)`; sub-pages are `Column { PharmListToolbar(title, onBack, actions?) ; weight(1f) content }`. No M3 `Scaffold`, no per-feature page wrapper — the app shell renders once in `MainShell`. | See `.claude/skills/kmp-layout-pattern/`. |
