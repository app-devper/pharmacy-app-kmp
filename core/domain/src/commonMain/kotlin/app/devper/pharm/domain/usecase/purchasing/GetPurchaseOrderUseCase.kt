package app.devper.pharm.domain.usecase.purchasing

import app.devper.pharm.domain.usecase.BaseUseCase

import app.devper.pharm.common.AppDispatchers
import app.devper.pharm.domain.model.PurchaseOrder
import app.devper.pharm.domain.repository.purchasing.PurchaseOrderRepository

class GetPurchaseOrderUseCase(private val repo: PurchaseOrderRepository, dispatchers: AppDispatchers) :
    BaseUseCase<String, PurchaseOrder>(dispatchers) {
    override suspend fun execute(param: String): PurchaseOrder = repo.get(param)
}
