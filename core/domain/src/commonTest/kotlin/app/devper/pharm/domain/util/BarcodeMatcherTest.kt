...
class BarcodeMatcherTest {
    ...
    fun returns_null_for_blank_code() {
        assertNull(BarcodeMatcher.match(listOf(drug("d1")), "  "))
    }
    ...
    fun matches_visible_alt_unit_barcode_first() {
        ...
        val match = BarcodeMatcher.match(listOf(d), "ALT-1")
    ...
    fun falls_back_to_primary_drug_barcode() {
        val match = BarcodeMatcher.match(listOf(drug("d1", barcode = "8851001")), "8851001")
        assertEquals("d1", match?.drug?.id)
    ...
    fun final_fallback_is_regNo() {
        val match = BarcodeMatcher.match(listOf(drug("d1", regNo = "FDA-12345")), "FDA-12345")
        assertEquals("d1", match?.drug?.id)
        assertNull(match?.altUnit)
    }
    ...
    fun returns_null_when_nothing_matches() {
        assertNull(BarcodeMatcher.match(listOf(drug("d1", barcode = "x", regNo = "y")), "nope"))
    }
